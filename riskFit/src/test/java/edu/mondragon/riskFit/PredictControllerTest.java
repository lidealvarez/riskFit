...
public class PredictControllerTest {
    ...
    public void testShowIndex() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get("/"))
                .andExpect(MockMvcResultMatchers.status().isOk()) // Verifica que la respuesta sea 200 OK
                .andExpect(MockMvcResultMatchers.view().name("index")); // Verifica que la vista sea "index"
    }

    @Test
    public void testPredictEndpoint() throws Exception {
        mockMvc.perform(MockMvcRequestBuilders.get("/predict"))
                .andExpect(MockMvcResultMatchers.status().isOk()) // Verifica que la respuesta es OK (200)
                .andExpect(MockMvcResultMatchers.view().name("predict")); // Verifica que la vista retornada es "predict"
    }
    ...
    public void testPostPrediction() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.post("/predict")
                        ...
                        .content(jsonInput))
                .andExpect(MockMvcResultMatchers.status().isOk()) // Verifica que la respuesta es OK (200)
                .andExpect(MockMvcResultMatchers.view().name("result")) // Verifica que la vista retornada es "result"
    ...
    public void testPredictWithError() throws Exception {
        ...
        mockMvc.perform(MockMvcRequestBuilders.post("/predict")
                ...
                .andExpect(MockMvcResultMatchers.view().name("error")) // Verifica que la vista es "error"
                .andExpect(MockMvcResultMatchers.model().attribute("error",
                        "Ocurrió un error al procesar la predicción: Error al procesar la predicción")); // Verifica que el mensaje de error esté en el modelo
    }
}
